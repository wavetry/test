...
p = re.compile(r'<key>memberVarAssignmentName</key>(\s+)<string>(\s+)</string>(\s+)<key>memberVarAssignmentType</key>(\s+)<integer>1</integer>')

def func(m):
	displayName = m.group(2)

	displayName = time.ctime()
	print("displayName" + displayName)
	retult = "<key>memberVarAssignmentName</key>%s<string>%s</string>%s<key>memberVarAssignmentType</key>%s<integer>1</integer>" % (m.group(1),displayName,m.group(3),m.group(4))
	return retult
